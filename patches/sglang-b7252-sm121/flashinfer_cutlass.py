...
def _unpack_topk_output(topk_output) -> tuple[torch.Tensor, torch.Tensor]:
    """Return explicit weights and expert ids for CUTLASS MoE.

    Inkling can prepack each route as ``(expert_id << 16) | bf16_weight_bits``
    for the TRTLLM routed backend. CUTLASS requires the two tensors separately,
    so invert the frozen ``PackTopkIds`` representation without recomputing
    routing or changing precision.
    """
...
def _run_flashinfer_cutlass(
    *,
    ...
    topk_weights, topk_ids = _unpack_topk_output(topk_output)
