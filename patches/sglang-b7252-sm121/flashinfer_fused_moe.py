...
def gen_cutlass_fused_moe_module(
    ...
    module_name = f"fused_moe_{device_arch}"
    aot_path = jit_env.FLASHINFER_AOT_DIR / module_name / f"{module_name}.so"
    if os.environ.get("FLASHINFER_DISABLE_JIT"):
        if not aot_path.exists():
            raise MissingJITCacheError(
                "JIT compilation is disabled via FLASHINFER_DISABLE_JIT, but "
                f"the required AOT module is missing: {aot_path}",
            )
    else:
        try:
